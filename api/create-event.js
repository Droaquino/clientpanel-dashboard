...
export default async function handler(req, res) {
  ...
  const { title, processName, date, sh, sm, eh, em, participantes } = req.body
    ...
    const event = await calendar.events.insert({
      ...
      requestBody: {
        ...
        description: [
          ...
          participantes?.length ? `Participantes: ${participantes.join(', ')}` : null,
          '',
        ...
        attendees: participantes?.length ? participantes.map(p => ({ displayName: p })) : [],
        conferenceData: {
          createRequest: {
